...
if __name__ == "__main__":
    ...
    for year in range(2002, 2018):
        ...
        dfy.rename(columns={cons.year_id: yid},
                inplace=True)
        if year == 2002:
            sd = ReferenceData.from_first_file(dfy, id=cons.sid, data_id=yid, starting_id=1, 
                                               null_flag_cols=[], add_cols=['F4FN', 'F4LN'], log=log)
        else:
            sd = (sd.reset_reference() \
                    .add_sup_data(dfy, data_id=yid, add_cols=['F4FN', 'F4LN'], one_to_one=False)
                    .loop_merge([Merge(custom_merges=cons.custom_merges, check_duplicates=False)])
                    .append_to_reference())
    ...
    log.info('Number of unique IDs = %d', len(sd.reference.df[cons.sid].unique()))
