...
'''clean script for complaints-accused_2000-2023_2023-08_p053540'''
...
if __name__ == "__main__":
    ...
    null_appointed_date = df['appointed_date'].isnull()

    df.loc[~null_appointed_date, 'merge'] = 1

    # use these to determine whether to merge or not here, otherwise the clean process will null most of these fields out/normalize them
    non_numeric_star = ~df['star'].str.isnumeric().fillna(False)
    # lower_case_rank = ~df['assigned_rank'].str.isupper().fillna(True)
    # lower_case_first_name = ~df['first_name'].str.isupper().fillna(True)
    # lower_case_last_name = ~df['last_name'].str.isupper().fillna(True)

    df.loc[null_appointed_date & (
        non_numeric_star 
        # lower_case_rank 
        # lower_case_first_name |
        # lower_case_last_name)
        ), 'merge'] = 0
    df = clean_data(df, log)
    
    # manually fix a suffix that didn't get split: LAUJR to LAU JR
    df.loc[(df['first_name_NS'] == 'GERALD') & 
           (df['last_name_NS'] == 'LAUJR'), ['last_name_NS', 'suffix_name']] = ('LAU', "JR")
