...
def get_interfile_changes(df, id, id_cols, incident_cols):
    """For files/dataframes where id/demographic data for the same officer can change within the file itself. 
    Most commonly when demographic data is point in time, e.g., where it'll show there badge number at the time of the incident represented by a row


    Parameters
    ----------
    df : DataFrame
        data with interfile changes. Assumes it's already been run through assign-unique-id
    id : str
        column for file id generated by assign_unique_ids function
    id_cols : list of str
        list of columns that represent id/demographic data
    incident_cols : list of str
        list of columns that represent each incident

    Returns
    -------
    possible_interfile_changes : dict {str: DataFrame}
        dict mapping change_column to possible changes for that column
    """    
    set_join = lambda x: ', '.join(set(x.astype(str)))

    all_cols = id_cols + incident_cols
    possible_changes = {}

    # consider each column as a possible change column, group by everything but that change column, look for duplicates
    for change_col in all_cols:
        id_col_subset = [col for col in all_cols if col != change_col]

        possible_changes[change_col] = df[[id] + all_cols].fillna("") \
            .groupby(id_col_subset, as_index=False) \
            .agg(ids=(id, set_join),
    ...
    # leave two columns out at a time (one from id, one from incident), do the same thing as above
    for incident_col in incident_cols:
        for change_col in id_cols:
            ...
            possible_changes[(incident_col, change_col)] = df \
                .loc[~df[id].isin(single_change_ids), all_cols + [id]] \
                .fillna("") \
                .groupby(id_cols, as_index=False) \
                .agg(ids=(id, set_join),
                    **{change_col: (change_col, set_join)},
...
def apply_interfile_change(df, change_row):
    """Given row with interfile change (from get_interfile_changes), apply that change

    Parameters
    ----------
    df : DataFrame
        dataframe with interfile changes
    change_row : Series
        row
    """    
