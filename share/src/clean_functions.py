...
def clean_dates(df):
    ...
    # Create dt_df as unique rows in input df
    dt_df = (df[df_cols]
             .drop_duplicates()
             .copy()
             .reset_index(drop=True))
    # Give dt_df an ID by index
    dt_df.insert(0, 'ID', dt_df.index)
    # Give ID to full data
    df = df.merge(dt_df, on=df_cols, how='inner')
    ...
    # Initialize empty cleaned_df
    cleaned_df = pd.DataFrame()
    # Iterate over stored date(time) columns
    for col in df_cols:
        # Store column prefix ([org, hire], [appointed])
        col_prefix_list = col.split('_')[:-1]
        ...
        # And create column in dt_df ending with _date
        dt_df[date_name] = dt_df[col].map(lambda x: x.split(' ')[0])
        try:
            cleaned_df[date_name] = \
                pd.to_datetime(dt_df[date_name], errors='raise').dt.date
        # If there were errors, notify the user,
        # And repeat above but with coercing errors to NaT
        except:
            print('Some errors in {}. Returned as NaT.'.format(date_name))
            cleaned_df[date_name] = \
                pd.to_datetime(dt_df[date_name], errors='coerce').dt.date
        ...
        cleaned_df[date_name] = \
            cleaned_df[date_name].map(lambda x:
        ...
        if 'time' in col:
            try:
                cleaned_df[time_name] = \
                    pd.to_datetime(dt_df[col]).dt.time
            except:
                print('Some errors in {}. Returned as NaT.'.format(col))
                cleaned_df[time_name] = \
                    pd.to_datetime(dt_df[col], errors='coerce').dt.time
    ...
    assert cleaned_df.shape[0] == df_rows,\
...
def clean_data(df, skip_cols=[]):
    ...
    for col_name, col_type in col_dict.items():
        ...
        # If col_type is date or datetime
            ...
            cleaned_date_df = clean_dates(df[[col_name]])
            del df[col_name]
