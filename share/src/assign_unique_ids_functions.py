...
def max_aggregate(df, uid, col):
    '''returns 2 column pandas dataframe
       ...
       if uid has only null values, it is returned
    ...
    # If duplicate dataframe is not empty
        ...
        return df.append(groups).reset_index(drop=True)
...
def mode_aggregate(df, uid, col):
    '''returns 2 column pandas dataframe
       ...
       if uid has only null values, it is returned
       if there are multiple modes, the first will be returned

       see test_assign_unique_ids_functions
    '''
    # Ignore nan filtering warning
